# check_whether_CF_applied_is_conservative_or_not_v0.py
...
list_of_obj_of_CF_COMP_input = [
    list_of_obj_of_all_result_objects[i]
    for i in range(len(list_of_obj_of_all_result_objects))
    if list_of_obj_of_all_result_objects[i].Name.Contains("CF_COMP")]
...
if len(obj_of_NS_parts_to_be_compensated) == 0:
    ...
    obj_of_NS_parts_to_be_compensated.Name = "NS_parts_to_be_compensated"
    exit()
...
if len(list_of_obj_of_CF_COMP_input)==0 and len(list_of_obj_of_CF_WT_input)==0:
    error_message = r"""Contour results  which contain "CF_COMP" and "CF_WT" prefix/suffix in their name are not defined. This is required for reading both "Node Number vs Value at Room Temperature" and "Node Number vs Value at Working Temperature" data. Please create both these objects in the tree and try again."""
    msg = Ansys.Mechanical.Application.Message(error_message, MessageSeverityType.Error)
    ExtAPI.Application.Messages.Add(msg)
    exit()
    
if len(list_of_obj_of_CF_COMP_input)==0:
    error_message = r"""A contour result  which contains "CF_COMP" prefix/suffix in its name is not defined.  This is required for reading the "Node Number vs Value at Working Temperature" data. Please create this object in the tree and try again."""
    msg = Ansys.Mechanical.Application.Message(error_message, MessageSeverityType.Error)
    ExtAPI.Application.Messages.Add(msg)
    exit()

if len(list_of_obj_of_CF_WT_input)==0:
    error_message = r"""A contour result  which contains "CF_WT" prefix/suffix in its name is not defined. This is required for reading the "Node Number vs Value at Room Temperature" data. Please create this object in the tree and try again."""
    msg = Ansys.Mechanical.Application.Message(error_message, MessageSeverityType.Error)
    ExtAPI.Application.Messages.Add(msg)
    exit()
    
if len(list_of_obj_of_CF_COMP_input)>1:
    error_message = r"""There cannot be more than one contour with "CF_COMP" prefix/suffix in its name in the tree."""
    msg = Ansys.Mechanical.Application.Message(error_message, MessageSeverityType.Error)
    ExtAPI.Application.Messages.Add(msg)
    exit()
...
if len(list_of_obj_of_CF_Temp_input)==0:
    ...
    ExtAPI.Application.Messages.Add(msg)
    solution_object_of_CF_COMP_result = list_of_obj_of_CF_COMP_input[0].Parent
    object_of_CF_Temp_result_created = solution_object_of_CF_COMP_result.AddUserDefinedResult()
...
if len(list_of_obj_of_CF_COMP_input)==1 and len(list_of_obj_of_CF_WT_input)==1 and len(list_of_obj_of_CF_Temp_input):
    ...
    obj_of_CF_COMP_input_data = list_of_obj_of_CF_COMP_input[0]
    solver_files_directory_CF_COMP_input = obj_of_CF_COMP_input_data.Parent.WorkingDir
    file_path_CF_COMP_input_txt = solver_files_directory_CF_COMP_input + "CF_COMP_input_data.txt"
    file_path_CF_COMP_input_csv = solver_files_directory_CF_COMP_input + "CF_COMP_input_data.csv"
    obj_of_CF_COMP_input_data.ExportToTextFile(file_path_CF_COMP_input_txt)
    
    # Open the space-separated text file and the CSV file
    with open(file_path_CF_COMP_input_txt, 'r') as text_file, open(file_path_CF_COMP_input_csv, 'wb') as csv_file:
        # Create a CSV writer object
        csv_writer = csv.writer(csv_file)
...
seqv_room_temp_file_path = r"{file_path_CF_WT_input_csv}"
seqv_working_temp_file_path = r"{file_path_CF_COMP_input_csv}"
...
print("SEQV_of_Node_at_Working_Temperature:" + str(SEQV_of_Node_at_Working_Temperature[0]))
print("reserve_factor_wt:" + str(reserve_factor_wt[0]))
print("Yield_Strength_at_Room_Temperature:" + str(Yield_Strength_at_Room_Temperature[0]))
print("target_seqv_at_room_temp:" + str(target_seqv_at_room_temp[0]))
print("SEQV_of_Node_at_Room_Temperature:" + str(SEQV_of_Node_at_Room_Temperature[0]))
print("Static_Compensation_Factor_for_Target_SEQV:" + str(Static_Compensation_Factor_for_Target_SEQV[0]))
...
solver_files_directory_CF_COMP_input = r"{solver_files_directory_CF_COMP_input}"

# Specify the filename you want to save the data to
output_filepath_of_static_CF_csv = os.path.join(solver_files_directory_CF_COMP_input, 'NodeID_vs_Static_CF_Result.csv')
...
""".format(solver_files_directory_CF_COMP_input=solver_files_directory_CF_COMP_input[:-1],
           file_path_CF_Temp_input_csv=file_path_CF_Temp_input_csv,
           file_path_CF_WT_input_csv=file_path_CF_WT_input_csv,
           file_path_CF_COMP_input_csv=file_path_CF_COMP_input_csv)

# Define the name of the cpython script to be run
cpython_script_name = "static_cf_calculation.py"
cpython_script_path = solver_files_directory_CF_COMP_input + cpython_script_name
...
solution_object_of_contour_object_of_Static_CF_result = obj_of_CF_COMP_input_data.Parent
